"""
tw_stock.py — hermesagent 台股查價 Tool (V1.7)
...
V1.7 修正：
  - [Bug] V1.6 的 _is_valid_price 用 float 比對 z 與股票代碼數字，會誤殺
    真實股價（例如 0050 真實股價來到 $50.00 時，會被誤判成異常值丟棄）。
    改為嚴格字串比對：只有 z 字串與股票代碼字串完全相同才視為異常，
    不再有數值碰巧相等時誤殺正常報價的風險。
"""
...
def _is_valid_price(z, stock_no) -> bool:
    """
    防呆：TWSE MIS API 在特定 ETF（如 009816）會把 z（最新成交價）欄位
    誤帶回股票代碼本身的字串。

    採用嚴格字串比對，而非轉成 float 比對數值：
    若改用 float，當真實股價剛好等於代碼的數字部分時（例如 0050 真實股價
    來到 $50.00，code=50.0、z=50.0），會把正常報價誤判為異常並丟棄。
    字串比對只在「完全相同」時才攔截，不會有此誤殺風險。
    """
    if not _is_numeric(z):
        return False
    if str(z).strip() == str(stock_no).strip():
        return False
    return True
